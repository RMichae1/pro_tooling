...
class Experiment:
    ...
    def derive_vae_mutations(self, sample_n=1):
        ...
        for seq, _, _ in sequence_dataset:
            samples.append(self.vae.latent_sample(seq.flatten(), n=sample_n).reshape(-1).detach().numpy())  # TODO why is there a sequence when sampling??
    ...
    def prepare_blat_experimental_data(self, load_existing=True) -> dict:
        if os.path.exists("./data/blat/blat_mutations.pkl") and load_existing:
            with open("./data/blat/blat_mutations.pkl", "rb") as filehandle:
