...
class VariationalAutoencoder:
    ...
    def __init__(self,
        ...
        if sparsity != False:
            self.params["W_out_scale-mu"] = theano.shared( \
                create_weight_zeros(int(self.final_output_size / self.n_patterns), \
                self.seq_len), \
                name="W_out_scale-mu")

            self.params["W_out_scale-log_sigma"] = theano.shared( \
                create_weight_logsig(int(self.final_output_size / self.n_patterns), \
                self.seq_len), \
        ...
        self.update, self.encode, self.decode, \
            ...
            self.get_pattern_activations, self.gradients \
    ...
    def create_gradientfunctions(self):
        ...
        grad_fun = theano.function([x], T.grad(T.mean(logpx_i), list(self.params.values())), allow_input_downcast=True)

        # Compute all the gradients
        gradients = T.grad(logpx_update, list(self.params.values()))
        ...
        return update, encode, decode, recognize, likelihoods,\
            all_likelihood_components, get_pattern_activations, grad_fun
