...
def likelihood(_vae: VAE, seq: np.array, idx: int, latent_sample: np.array) -> np.array:
    ...
    p_z = Normal(loc=torch.zeros(1), scale=torch.ones(1)).log_prob(latent_sample).sum(1).exp().detach().numpy()[np.newaxis, :]
    z_x_loc, z_x_scale = _vae.encoder(oh_x)
    # transform latent sample z' => z
    z = z_x_loc + torch.Tensor(latent_sample)*torch.sqrt(z_x_scale)
    q_z_x = Normal(z_x_loc, z_x_scale).log_prob(z).sum(1).exp().detach().numpy()[np.newaxis, :]
    ll_p_x_z = Categorical(_vae.decoder(z).exp()).log_prob(torch.Tensor(seq)).permute(1, 0).detach().numpy()[np.newaxis, :]
    # print(ll_p_x_z.shape)
    # p_x_z_not_i = np.exp(ll_p_x_z[:, idx+1:, seq[:, idx+1:]]) if idx == 0 else \
    #     np.exp(ll_p_x_z[:, :idx, seq[:, :idx]] + ll_p_x_z[:, idx+1:, seq[:, idx+1:]])
    # print(ll_p_x_z[:, idx+1:, seq[:, idx+1:]])
    # p(x_i==a) is exp(log_likelihood(p_x[n_sample, position, residue]))
    p_x_i_x_not_i = (1/n_samples) * (np.exp(ll_p_x_z[:, idx, seq[:, idx]]) * p_z) / q_z_x
    return np.array(p_x_i_x_not_i)


def naive_v_K(seq: np.ndarray, adj: np.ndarray, vae: VAE, sample_size=1) -> np.ndarray:
    """
    ...
    for p in range(n):
        for q in range(n):
            for idx in range(seq.shape[1]):
                ...
                temp_K.fill(0.)
                for l in nbps:
                    p_val = likelihood(vae, seq, l, latent_sample)
                    for i, val in enumerate(p_val[0]):
                        temp_K[i, p, q] += val
                p_val = likelihood(vae, seq, idx, latent_sample)
                for i, val in enumerate(p_val[0]):
                    temp_K[i, p, q] *= val
                K += temp_K
    print(K)
